//var profileControllers = angular.module("profileControllers", ['ngAnimate']);

myApp.controller("HomeController", ['$scope','$interval','$timeout','$rootScope', function($scope, $interval, $timeout, $rootScope){
  ...
  $scope.description = function() {
        $describeWrap = $('#page-home .describe-wrapper');
        $visibleSpan = $('#page-home .describe-wrapper span.is-visible');
        $nextVisible = null;
        $rootScope.needStop = false;

        $describeWrap.css('width',$('#page-home .describe-wrapper span.is-visible').width() + 45);
        //$scope.startInfo = $interval(function(){
        $scope.showDesc = function() {
          $describeWrap.animate({
            width: "0px"
          }, 1000, function() {
            if($('#page-home .describe-wrapper span.is-visible').next().length > 0) {
              $nextVisible = $('#page-home .describe-wrapper span.is-visible').next();
            } else {
              $nextVisible = $('#page-home .describe-wrapper span:first-child');
            }
            $('#page-home .describe-wrapper span.is-visible').removeClass('is-visible').addClass('is-hidden');
            $nextVisible.removeClass('is-hidden').addClass('is-visible');
            $describeWrap.animate({width: $('#page-home .describe-wrapper span.is-visible').width() + 5});
            $timeout(function(){
              if($rootScope.needStop) {
                return false;
              } else {
                $scope.showDesc();
              }
        ...
        $rootScope.$on("CallDescriptionMethod", function(){
           $scope.showDesc();
        });
  };
}]);
...
myApp.controller("ProjectController", ['$scope','$rootScope', function($scope, $rootScope){
	...
	function openItem(item) {
		...
		if( bodyScale ) {
			dynamics.animate(mainEl[0], { scale: bodyScale }, { type: dynamics.easeInOut, duration: 500 });
		}
		// after the transition is finished:
		onEndTransition(zoomer, function() {
			// reset body transform
			if( bodyScale ) {
				dynamics.stop(mainEl[0]);
				dynamics.css(mainEl[0], { scale: 1 });

				// fix for safari (allowing fixed children to keep position)
				mainEl[0].style.WebkitTransform = 'none';
				mainEl[0].style.transform = 'none';
			}
			// no scrolling
	...
	function closeContent() {
		...
		if( bodyScale ) {
			...
			mainEl[0].style.WebkitTransform = '';
			mainEl[0].style.transform = '';
		}
		...
		onEndTransition(contentItem, function(ev) {
			$(this).removeClass('content__item--reset');
			...
			setTimeout(function() {
				$(zoomer).removeClass('zoomer--notrans');
				$(zoomer).removeClass('zoomer--active');
				zoomer.style.WebkitTransform = 'translate3d(0,0,0) scale3d(1,1,1)';
				zoomer.style.transform = 'translate3d(0,0,0) scale3d(1,1,1)';
			}, 25);

			if( bodyScale ) {
				dynamics.css(mainEl[0], { scale: bodyScale });
				dynamics.animate(mainEl[0], { scale: 1 }, {
	...
	function navigate(dir) {
		...
		dynamics.animate(nextEl, { opacity: 1, translateX: 0 }, {
			...
			friction: 600,
			complete: function() {
				items.forEach(function(item) { $(item).removeClass('slide--current'); });
    ...
    $rootScope.$on("CallProjectMethod", function(){
       init();
    });
	//init();
...
// profileControllers.controller("AboutController", ['$scope', '$http', '$routeParams', function($scope, $http, $routeParams){
//
// }]);
...
function description() {
  var temp = ["Mobile Apps", "Web Sites"];
  var $ele = $(".home-content .describe span");
  var j=0, i=0;
  var chars = temp[0].split("");
  typedesc(i,j,temp,$ele,chars);
}

function typedesc(i,j,temp,$ele,chars) {
  if(i < temp.length) {
    if(j<chars.length) {
      if(!$ele.is(":visible")) {
        $ele.fadeIn();
      }
      $ele.html($ele.html() + chars[j]);
      j++;
      setTimeout(function(){
        typedesc(i,j,temp,$ele,chars);
      },250);
